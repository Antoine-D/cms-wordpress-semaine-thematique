 ...
 * @return string
 */
function travelop_lite_set_sidebar_position( $position ) {
	$queried_obj = apply_filters( 'travelop_lite_queried_object_id', false );
...
add_filter( 'theme_mod_sidebar_position', 'travelop_lite_set_sidebar_position' );
 ...
 * @return string
 */
function travelop_lite_render_macros( $string ) {

	$macros = apply_filters( 'travelop_lite_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
		'/%%date%%/' => date( get_option( 'date_format' ) ),
		'/%%privacy-policy%%/' => travelop_lite_get_privacy_link(),
	) );
 ...
 * @return string
 */
function travelop_lite_get_privacy_link() {
 ...
 * @return string
 */
function travelop_lite_render_icons( $content ) {

	$icons     = travelop_lite_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'travelop_lite_render_icons_callback', $content );
}
 ...
 * @return string
 */
function travelop_lite_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function travelop_lite_get_render_icons_set() {
	return apply_filters( 'travelop_lite_render_icons_set', array(
 ...
 * @return string
 */
function travelop_lite_render_theme_url( $url ) {
 ...
 * @return string
 */
function travelop_lite_get_thumbnail_size() {
 ...
 */
function travelop_lite_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function travelop_lite_post_formats_gallery() {
	if ( ! in_array( get_theme_mod( 'blog_layout_type' ), array( 'masonry-2-cols', 'masonry-3-cols' ) ) ) {
		return do_action( 'cherry_post_format_gallery', array(
			'size' => travelop_lite_get_thumbnail_size(),
		) );
	}

	$images = travelop_lite_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images(false);
