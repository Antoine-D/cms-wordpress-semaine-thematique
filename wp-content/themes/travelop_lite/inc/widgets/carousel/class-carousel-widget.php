 ...
 */
if ( ! class_exists( 'Travelop_lite_Carousel_Widget' ) ) {

	class Travelop_lite_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass		= 'travelop_lite widget_carousel';
			$this->widget_description	= esc_html__( 'Display a list of your posts on your site.', 'travelop_lite' );
			$this->widget_id			= 'widget_carousel';
			$this->widget_name			= esc_html__( 'Carousel', 'travelop_lite' );
			$this->settings				= array(
				'title' => array(
					'type'				=> 'text',
					'value'				=> esc_html__( 'Carousel', 'travelop_lite' ),
				...
				'post_cats' => array(
					...
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display categories', 'travelop_lite' ),
				),
				'post_comments' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display comments', 'travelop_lite' ),
				),
				'content' => array(
					'type'				=> 'switcher',
					'value'				=> 'false',
				...
				'slides_per_view' => array(
					...
					'min_value'			=> 1,
					'value'				=> 5,
					'label'				=> esc_html__( 'Number of slides per view', 'travelop_lite' ),
				),
				'slides_per_group' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Number slides per group', 'travelop_lite' ),
				),
				'slides_per_column' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Multi Row Slides Layout', 'travelop_lite' ),
				),
				'space_between_slides' => array(
					...
					'min_value'			=> 0,
					'value'				=> 30,
					'label'				=> esc_html__( 'Width of the space between slides(px)', 'travelop_lite' ),
				),
				'duration_speed' => array(
					...
					'min_value'			=> 100,
					'value'				=> 500,
					'label'				=> esc_html__( 'Duration of transition between slides (ms)', 'travelop_lite' ),
				),
				'navigation' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider navigation', 'travelop_lite' ),
				),
				'pagination' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider pagination', 'travelop_lite' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', travelop_lite_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="travelop_lite-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h4>' . esc_html__( 'Posts not found', 'travelop_lite' ) . '</h4>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'travelop_lite_carousel_default_query_args', array(
				'post_type'			=> 'post',
				'orderby'			=> 'date',
				'order'				=> 'DESC',
				'posts_per_page'	=> -1,
		...
		public function get_carousel_loop( $posts_query ) {
			...
			if ( ! $view_dir ) {
				return '<h4>' . esc_html__( 'View file not found', 'travelop_lite' ) . '</h4>';
			}
	...
	function tm_register_carosel_widgets() {
		register_widget( 'Travelop_lite_Carousel_Widget' );
	}
}