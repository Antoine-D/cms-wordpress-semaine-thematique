 ...
 * @return array
 */
function talkingbusiness_lite_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! talkingbusiness_lite_is_meta_visible( $meta, $context ) ) {
		return;
	}
	 ...
	 * $6%s - `thumbnail`
	 */
	$defaults = apply_filters( 'talkingbusiness_lite_default_args_share_buttons', array(
		'facebook' => array(
			'icon'      => 'fa fa-facebook',
			'name'      => esc_html__( 'Facebook', 'talkingbusiness_lite' ),
			'share_url' => 'https://www.facebook.com/sharer/sharer.php?u=%3$s&t=%4$s',
		),
		'twitter' => array(
			'icon'      => 'fa fa-twitter',
			'name'      => esc_html__( 'Twitter', 'talkingbusiness_lite' ),
			'share_url' => 'https://twitter.com/intent/tweet?url=%3$s&text=%4$s',
		),
		'google-plus' => array(
			'icon'      => 'fa fa-google-plus',
			'name'      => esc_html__( 'Google+', 'talkingbusiness_lite' ),
			'share_url' => 'https://plus.google.com/share?url=%3$s',
		),
		'linkedin' => array(
			'icon'      => 'fa fa-linkedin',
			'name'      => esc_html__( 'LinkedIn', 'talkingbusiness_lite' ),
			'share_url' => 'http://www.linkedin.com/shareArticle?mini=true&url=%3$s&title=%4$s&summary=%5$s&source=%3$s',
		),
		'pinterest' => array(
			'icon'      => 'fa fa-pinterest',
			'name'      => esc_html__( 'Pinterest', 'talkingbusiness_lite' ),
	...
	$default_config = apply_filters( 'talkingbusiness_lite_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'talkingbusiness_lite_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'talkingbusiness_lite' ) . $network['name'],
	...
	if( $meta === 'single_post_share_buttons' ){
		$share_label = get_theme_mod( 'single_post_share_label', talkingbusiness_lite_theme()->customizer->get_default( 'single_post_share_label' ) );
