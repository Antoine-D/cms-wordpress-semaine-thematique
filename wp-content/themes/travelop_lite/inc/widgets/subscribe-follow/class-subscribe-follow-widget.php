...
class Travelop_lite_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'travelop_lite widget-subscribe';
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'travelop_lite' );
		$this->widget_id          = 'travelop_lite_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'travelop_lite' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'travelop_lite' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'travelop_lite' ),
				'label' => esc_html__( 'Subscribe Title', 'travelop_lite' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'travelop_lite' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'travelop_lite' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'travelop_lite' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'travelop_lite' ),
				'label' => esc_html__( 'Subscribe submit label', 'travelop_lite' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'travelop_lite' ),
				'label' => esc_html__( 'Subscribe success', 'travelop_lite' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'travelop_lite' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'travelop_lite' ),
				'label' => esc_html__( 'Follow Title', 'travelop_lite' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'travelop_lite' ),
			),
		);

		add_action( 'wp_ajax_travelop_lite_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_travelop_lite_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'travelop_lite' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'travelop_lite' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'travelop_lite' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'travelop_lite' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		$follow_template = locate_template( 'inc/widgets/subscribe-follow/view/follow-view.php' );
		...
		$subscribe_template = locate_template( 'inc/widgets/subscribe-follow/view/subcribe-view.php' );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'travelop_lite' );
		}
	...
	public function get_social_nav() {
		return travelop_lite_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = travelop_lite_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'travelop_lite_subscribe' ) ) {
...
add_action( 'widgets_init', 'travelop_lite_register_subscribe_follow_widgets' );
function travelop_lite_register_subscribe_follow_widgets() {
	register_widget( 'Travelop_lite_Subscribe_Follow_Widget' );
}
