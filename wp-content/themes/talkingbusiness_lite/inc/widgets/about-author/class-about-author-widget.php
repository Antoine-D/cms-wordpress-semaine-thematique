 ...
 */
if ( ! class_exists( 'TalkingBusiness_Lite_About_Author_Widget' ) ) {

	class TalkingBusiness_Lite_About_Author_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'talkingbusiness_lite widget-about-author';
			$this->widget_description = esc_html__( 'Display an information about selected user.', 'talkingbusiness_lite' );
			$this->widget_id          = 'talkingbusiness_lite_widget_about_author';
			$this->widget_name        = esc_html__( 'About Author', 'talkingbusiness_lite' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'About Author', 'talkingbusiness_lite' ),
				...
				'user_id' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select user to show', 'talkingbusiness_lite' ),
				),
				'avatar_size' => array(
					'type'       => 'slider',
					'max_value'  => 512,
					...
					'value'      => 250,
					'step_value' => 1,
					'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'talkingbusiness_lite' ),
				),
				'avatar_img' => array(
					'type'               => 'media',
					'value'              => '',
					'multi_upload'       => false,
					'library_type'       => 'image',
					'upload_button_text' => esc_html__( 'Select image', 'talkingbusiness_lite' ),
					'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'talkingbusiness_lite' ),
				),
				'link' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Link (leave empty to hide)', 'talkingbusiness_lite' ),
				),
				'link_text' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Read More', 'talkingbusiness_lite' ),
					'label' => esc_html__( 'Link label', 'talkingbusiness_lite' ),
				),
			);
		...
		public function get_users_list() {
			...
			$result = array( '0' => esc_html__( 'Select a user', 'talkingbusiness_lite' ) );
		...
		public function get_author_avatar() {
			...
			if ( ! empty( $this->instance['avatar_img'] ) ) {
				$avatar_src = wp_get_attachment_image_src( intval( $this->instance['avatar_img'] ), 'talkingbusiness_lite-author-avatar' );
...
add_action( 'widgets_init', 'talkingbusiness_lite_register_about_author_widgets' );
function talkingbusiness_lite_register_about_author_widgets() {
	register_widget( 'TalkingBusiness_Lite_About_Author_Widget' );
}
