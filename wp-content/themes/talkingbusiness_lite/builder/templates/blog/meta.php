...
<div class="tm_pb_post_meta"><?php
	...
	if ( 'on' === $this->_var( 'show_date' ) ) {
		echo tm_get_safe_localization(
			sprintf(
				esc_html__( '%s', 'talkingbusiness_lite' ),
	...
	if ( 'on' === $this->_var( 'show_author' ) ) {
		echo tm_get_safe_localization(
			sprintf(
				'<span>' .  esc_html__( 'Posted by %s', 'talkingbusiness_lite' ) . '</span>',
	...
	if ( 'on' === $this->_var( 'show_comments' ) ) {
		...
		printf(
			esc_html(
				_nx( '1 Comment', '%s Comments', get_comments_number(), 'number of comments', 'talkingbusiness_lite' )
			),
