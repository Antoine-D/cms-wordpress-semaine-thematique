 ...
 * @return string
 */
function talkingbusiness_lite_set_post_meta_value( $value ) {
	$queried_obj = apply_filters( 'talkingbusiness_lite_queried_object_id', false );
	...
	$curr_opions = 'talkingbusiness_lite_' . str_replace( 'theme_mod_', '', current_filter() );
 ...
 * Sidebar position
 */
 add_filter( 'theme_mod_sidebar_position', 'talkingbusiness_lite_set_post_meta_value' );
 
 /**
 * Header container type
 */
 add_filter( 'theme_mod_header_container_type', 'talkingbusiness_lite_set_post_meta_value' );
 
 /**
 * Content container type
 */
 add_filter( 'theme_mod_content_container_type', 'talkingbusiness_lite_set_post_meta_value' );
 
 /**
  * Footer container type
  */
 add_filter( 'theme_mod_fotter_container_type', 'talkingbusiness_lite_set_post_meta_value' );
 ...
 * @return string
 */
function talkingbusiness_lite_render_macros( $string ) {

	$macros = apply_filters( 'talkingbusiness_lite_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function talkingbusiness_lite_render_icons( $content ) {
	$icons     = talkingbusiness_lite_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'talkingbusiness_lite_render_icons_callback', $content );
}
 ...
 * @return string
 */
function talkingbusiness_lite_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function talkingbusiness_lite_get_render_icons_set() {
	return apply_filters( 'talkingbusiness_lite_render_icons_set', array(
 ...
 * @return string
 */
function talkingbusiness_lite_render_theme_url( $url ) {
 ...
 */
function talkingbusiness_lite_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function talkingbusiness_lite_post_formats_gallery() {
	$size = talkingbusiness_lite_post_thumbnail_size();
	...
	$images = talkingbusiness_lite_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images(false);
 ...
 * @return bool
 */
function talkingbusiness_lite_is_meta_visible( $meta, $context = 'loop' ) {

	if ( ! $meta ) {
		return false;
	}

	$meta_enabled = get_theme_mod( $meta, talkingbusiness_lite_theme()->customizer->get_default( $meta ) );
 ...
 * @return array
 */
function talkingbusiness_lite_post_thumbnail_size( $args = array() ) {
	$args = wp_parse_args( $args, array(
		'small'        => 'post-thumbnail',
		'fullwidth'    => 'talkingbusiness_lite-thumb-l',
		'class_prefix' => '',
	) );

	$layout      = get_theme_mod( 'blog_layout_type', talkingbusiness_lite_theme()->customizer->get_default( 'blog_layout_type' ) );
	$format      = get_post_format();
	$size_option = get_theme_mod( 'blog_featured_image', talkingbusiness_lite_theme()->customizer->get_default( 'blog_featured_image' ) );
