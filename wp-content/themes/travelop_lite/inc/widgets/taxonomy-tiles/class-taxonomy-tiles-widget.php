...
if ( ! class_exists( 'Travelop_lite_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class Travelop_lite_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_name			= esc_html__( 'Taxonomy Tiles', 'travelop_lite' );
			$this->widget_description	= esc_html__( 'This widget displays images from taxonomy.', 'travelop_lite' );
			...
			$this->settings = array(
				'title'	=> array(
					...
					'label'				=> esc_html__( 'Widget title', 'travelop_lite' ),
				),
				'terms_type' => array(
					'type'				=> 'radio',
					'value'				=> 'category',
				...
				'category'=> array(
					...
					'label'				=> esc_html__( 'Select category to show', 'travelop_lite' ),
				...
				'post_tag' => array(
					...
					'label'				=> esc_html__( 'Select tags to show', 'travelop_lite' ),
				...
				'description_length' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Description words length ( Set 0 to hide description. )', 'travelop_lite' ),
				),
				'show_post_count' => array(
					...
					'options'		=> array(
						'show_post_count_check' => esc_html__( 'Show post count', 'travelop_lite' ),
					),
				),
				'layout_type' => array(
					'type'				=> 'radio',
					...
					'options'			=> array(
						'grid' => array(
							'label'		=> esc_html__( 'Grid', 'travelop_lite' ),
							'slave'		=> 'layout_type_grid',
						),
						'tiles' => array(
							'label'		=> esc_html__( 'Tiles', 'travelop_lite' ),
						),
					),
					'label'				=> esc_html__( 'Choose Layout Type', 'travelop_lite' ),
				),
				'columns_number' => array(
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Columns number ( layout type grid )', 'travelop_lite' ),
				...
				'items_padding' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Items padding ( size in pixels )', 'travelop_lite' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				echo apply_filters( 'travelop_lite_taxonomy_tiles_widget_before', '<div class="row grid ' . $layout_type . '-columns columns-number-' . $columns_number . '" style="margin: 0 0 0 -' . $items_padding . 'px" >', $instance );
				...
				foreach ( $terms as $term_key => $term ) {
					...
					if ( $view_dir ){
						...
						$count = $this->utility->meta_data->get_post_count_in_term( array(
								...
								'sufix'			=> _n( '%s post', '%s posts', $term->count, 'travelop_lite' ),
							),
							$term->term_id
						);
				...
				echo apply_filters( 'travelop_lite_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'travelop_lite_register_taxonomy_tiles_widget' );
	function travelop_lite_register_taxonomy_tiles_widget() {
		register_widget( 'Travelop_lite_Taxonomy_Tiles_Widget' );
	}
